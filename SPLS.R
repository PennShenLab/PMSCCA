SPLS<-function(X, Y, standardize="Center", c1=NULL,c2=NULL, u0=NULL,v0=NULL, maxiter=100, tolvar=10e-3, tolfun=10e-6, QUIET=F,autoT=F){
  # --------------------------------------------------------------------
  # SPLS algorithm 
  # --------------------------------------------------------------------
  
  # SPLS model:
  #     maximize     (1-lambda) * 1/(n-1) * u' * X' * Y * v 
  ...
  output<-PM_SPLS(X, Y, E=matrix(0,nrow = ncol(X),ncol = ncol(Y)), standardize=standardize, c1=c1,c2=c2,lambda=0, u0=u0,v0=v0, maxiter=maxiter, tolvar=tolvar, tolfun=tolfun, QUIET=QUIET,autoT=autoT)
